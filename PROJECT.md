...
- `research_state.json`: Non-authoritative cross-session world-model — question, open questions, assumptions, dead ends (orientation only, never findings; maintained by `beril state`/`whereami`, never gates a lifecycle transition)
...
The planning and execution arc is split into dedicated entry points: `/research-plan` writes the frozen, pre-registered `RESEARCH_PLAN.md` (`exploration → proposed`), `/execute-plan` runs it (`proposed → active → analysis`), and `/whereami` shows where a project stands and what to do next. `/berdl_start` orchestrates and routes a resumed project to the right stage by `beril.yaml.status`. The `beril state` (get/set the world-model) and `beril whereami` CLI verbs maintain and surface `research_state.json`. See [docs/scientific-planning.md](docs/scientific-planning.md) for the constructs and the three-plane authority model.
