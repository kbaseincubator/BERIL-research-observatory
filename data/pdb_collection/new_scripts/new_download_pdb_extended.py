...
Usage:
  ...
  python3 download_pdb_extended.py --output-dir ./extended_data --sample 100
...
def batch_graphql(pdb_ids, query_template, batch_size=1000):
    ...
    for i in range(0, total, batch_size):
        ...
        for entry in entries:
            if entry is None:
                print("Warning: Received null entry, skipping")
                continue
            yield entry
...
def download_taxonomy(pdb_ids, output_path, batch_size=1000):
    ...
    with open(output_path, "w", newline="") as f:
        ...
        for entry in batch_graphql(pdb_ids, TAXONOMY_LIGANDS_QUERY, batch_size):
            ...
            wrote = False  # track if anything valid is written
            seen = set()

            for pe in entry.get("polymer_entities") or []:
                for org in pe.get("rcsb_entity_source_organism") or []:
                    organism = org.get("ncbi_scientific_name") or None
            ...
            if not wrote:
                writer.writerow([pdb_id, "None", "None"])
                count += 1
