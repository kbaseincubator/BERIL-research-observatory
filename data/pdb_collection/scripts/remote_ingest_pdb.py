#!/usr/bin/env python3
"""
Ingest PDB metadata into the Iceberg lakehouse via Spark Connect (remote).

Reads TSVs from MinIO bronze storage and writes Iceberg tables to
kescience.pdb using Spark Connect through the SSH tunnel proxy.
...
Usage:
  /tmp/berdl_py313/bin/python3 scripts/remote_ingest_pdb.py
"""
...
BRONZE_BASE = "s3a://cdm-lake/tenant-general-warehouse/kescience/datasets/pdb"
TENANT = "kescience"
NAMESPACE = "pdb"
...
TABLES = [
    {
        "name": "pdb_entries",
        "file": f"{BRONZE_BASE}/pdb_entries.tsv",
        "schema": (
            "pdb_id STRING, title STRING, method STRING, method_full STRING, "
            "resolution FLOAT, r_work FLOAT, r_free FLOAT, organism STRING, "
            "deposition_date STRING, release_date STRING, citation_doi STRING"
        ),
    },
    {
        "name": "pdb_uniprot_mapping",
        "file": f"{BRONZE_BASE}/pdb_uniprot_mapping.tsv",
        "schema": (
            "pdb_id STRING, chain_id STRING, uniprot_accession STRING, "
            "res_beg INT, res_end INT, pdb_beg STRING, pdb_end STRING, "
            "sp_beg INT, sp_end INT"
        ),
    },
]
