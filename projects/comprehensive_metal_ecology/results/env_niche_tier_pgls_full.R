#!/usr/bin/env Rscript
# Tier split PGLS for all 33 env niche breadth responses — full n≈1574
# response ~ ko_per_mb_tier1_z + ko_per_mb_tier2_z + genome_mb_z
# Uses tier z-scores from kbase.ke_pangenome (same source as primary P1).

library(ape)
library(nlme)

DATA_DIR  <- "projects/comprehensive_metal_ecology"
TREE_FILE <- "projects/microbeatlas_metal_ecology/data/gtdb_bac_genus_pruned.tree"

cat("Loading tier z-scores (full 1574-genus set)...\n")
tier_df <- read.csv(file.path(DATA_DIR, "data/tier_z_scores_full.csv"),
                    stringsAsFactors=FALSE)
cat(sprintf("  Tier data: %d genera\n", nrow(tier_df)))

cat("Loading env niche PGLS input (33 SD responses)...\n")
env_df <- read.csv(file.path(DATA_DIR, "results/env_niche_all_pgls_input.csv"),
                   stringsAsFactors=FALSE)
cat(sprintf("  Env niche: %d genera\n", nrow(env_df)))

cat("Merging on genus_lower...\n")
df <- merge(env_df, tier_df[, c("genus_lower","ko_per_mb_tier1_z","ko_per_mb_tier2_z")],
            by="genus_lower", all.x=FALSE, all.y=FALSE)
cat(sprintf("  Merged: %d genera\n\n", nrow(df)))
...
run_tier_model <- function(data, tree, response_col, label_prefix) {
  fml_str <- sprintf("%s ~ ko_per_mb_tier1_z + ko_per_mb_tier2_z + genome_mb_z",
                     response_col)
  ...
  tryCatch({
    ...
    for (i in 1:nrow(co)) {
      cat(sprintf("  %-25s beta=%9.4f SE=%8.4f t=%7.3f p=%.4f\n",
                  rownames(co)[i], co[i,1], co[i,2], co[i,3], co[i,4]))
    }
    return(list(label=label, response=response_col, n=n_fit, lambda=lam,
                coef=as.data.frame(co)))
  }, error=function(e) {
    ...
    tryCatch({
      ...
      cat(sprintf("  OLS fallback: n=%d, lambda=0 (fixed)\n", n_fit))
      return(list(label=label, response=response_col, n=n_fit, lambda=0,
                  coef=as.data.frame(co)))
    }, error=function(e2) {
...
# ── Response variables (same 33 as primary env niche PGLS) ───────────────────
sd_responses <- list(
...
write.csv(out,
          file.path(DATA_DIR, "results/env_niche_tier_pgls_full_results.csv"),
          row.names=FALSE)
cat(sprintf("\nSaved env_niche_tier_pgls_full_results.csv (%d rows)\n", nrow(out)))
