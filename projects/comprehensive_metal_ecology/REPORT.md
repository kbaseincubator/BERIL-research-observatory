...
#### MicrobeAtlas v2 rerun: NB27 inverse PGLS (2026-08-08)

The inverse PGLS (NB27; `ko_per_mb_primary ~ niche characteristics`) was rerun substituting
v2 global `n_envs_detected` (mean across OTUs mapped to each genus, 591K env-only samples,
12 Env_Level_1 categories) for the v1 `n_biomes` predictor (42K Australian-biased MicrobeAtlas
samples). All other predictors (metal ranges, temperature range, pH) are unchanged — they are
computed from the NGSA-Australia feature matrix (42K samples, unaffected by global v2
expansion). Single-predictor PGLS results (n≈1,443–1,543 bacteria, λ≈0.86–0.91):

| Predictor | β | p | n | λ | V1 β (ref) | Note |
|-----------|---|---|---|---|-----------|------|
| **n_biomes_z (v1)** | **+0.215** | **≈0** | 1,475 | 0.905 | +0.215 | Dominant predictor in v1 |
| **n_envs_v2_z (v2)** | **−0.148** | 3.0×10⁻¹⁰ | 1,543 | 0.860 | — | **SIGN REVERSAL vs v1** |
| temp_range_z | +0.160 | 5.3×10⁻¹⁴ | 1,475 | 0.887 | +0.161 | Robust (unchanged) |
| Zn_range_z | +0.153 | 4.1×10⁻¹² | 1,475 | 0.890 | +0.153 | Robust (unchanged) |
| Cu_range_z | +0.147 | 1.2×10⁻¹¹ | 1,466 | 0.897 | +0.147 | Robust (unchanged) |
| precip_range_z | +0.085 | 2.3×10⁻⁴ | 1,475 | 0.885 | +0.073 | Similar |
| mean_Pb_z | +0.050 | 0.022 | 1,470 | 0.869 | +0.012 | Weak, inconsistent |
| sd_pH_z | +0.048 | 0.038 | 1,468 | 0.870 | +0.040 | Similar |

**Key finding: n_envs_v2_z has the opposite sign to v1 n_biomes_z** (β=−0.148 vs +0.215).
V1 n_biomes and v2 n_envs are only moderately correlated (Pearson r=0.417, Spearman ρ=0.395,
n=1,458 genera; 2026-08-09 diagnostic), indicating they rank genera very differently.

**Mechanistic diagnosis of the sign reversal (2026-08-09; `scripts/nb27_sign_reversal_diagnostic.py`):**

Genome size confounding is **not** the mechanism: both metrics correlate nearly identically with
mean genome size (n_biomes vs genome: Spearman ρ=+0.201; n_envs_v2 vs genome: ρ=+0.192;
n=1,458 genera). The genome-size distributions of high-ranking genera do not differ between
the two metrics.

The reversal is explained by **which taxa score high on each metric**:

*n_biomes inflated by host-associated versatile generalists.* Genera where n_biomes ≫ n_envs_v2
include *Streptococcus* (KO/Mb=34.6), *Staphylococcus* (22.4), *Corynebacterium* (19.9), and
*Bacillus* (14.1) — all human-associated commensal/pathogenic bacteria detected across multiple
human body-site samples in the AusMicrobiome 42K dataset (skin, oral, gut, and environmental).
These taxa have large, high-density gene repertoires because they colonise multiple host niches
via HGT-acquired functions. AusMicrobiome includes body-site samples under its "host-associated"
biome category, so broad-biome taxa in v1 are disproportionately host generalists with high KO
density → positive β.

*n_envs_v2 captures genuine biogeochemical environmental cosmopolitans.* Genera where
n_envs_v2 ≫ n_biomes include *Vallitalea* (n_biomes=1, KO/Mb=5.2), *Saprospira* (1, 2.6),
*Caldilinea* (3, 8.3), and *Synergistes* (3, 7.5) — obligate anaerobes and marine specialists
undetected in most Australian body-site or agricultural samples but widespread in global aquatic,
sediment, and soil environments. These have minimal-to-moderate gene repertoires optimised for
narrow biogeochemical functions → negative β.

**Partial correlation test:** Controlling for log_genome_size, the opposite signs persist:
n_biomes ↔ KO/Mb | genome: r=+0.251 (p=2.5×10⁻²²); n_envs_v2 ↔ KO/Mb | genome: r=−0.097
(p=1.9×10⁻⁴). Even within genera of identical genome size, n_biomes selects for high-KO-density
taxa and n_envs_v2 selects for low-KO-density taxa — the reversal is qualitative, not a
genome-size artefact.

**Multi-predictor comparison:**

| Model | n | λ | R² | n_biomes / n_envs β | temp_range β |
|-------|---|---|----|--------------------|-------------|
| v1 (n_biomes_z) | 1,460 | 0.901 | 0.063 | n_biomes: β=+0.193, p=5×10⁻⁹ | +0.053 (p=0.055, ns) |
| v2 (n_envs_v2_z) | 1,443 | 0.871 | 0.072 | n_envs: β=−0.177, p=7×10⁻¹³ | +0.163, p=5×10⁻¹⁴ |

The temperature range signal — masked in v1 by collinearity with n_biomes — emerges as the
dominant significant covariate in the v2 multi-predictor model (β=+0.163, p=5×10⁻¹⁴), while
n_envs_v2 is strongly negative (β=−0.177, p=7×10⁻¹³). Metal range signals (Cu, Zn) remain
positive in both models.

**Consistency with the forward PGLS:** The v2 inverse result (broader genuine environmental
breadth → fewer KOs/Mb) is mechanistically consistent with the forward PGLS (more KOs/Mb →
narrower niche breadth, β=−0.031). The v1 n_biomes result was the anomaly — driven by
host-associated taxa with high KO density inflating the Australian biome-count metric.

**Implications:** The NB27 v1 conclusion — that broader biome range positively predicts KO
density — is dataset-specific (42K Australian samples with host-associated body-site sampling)
and does not replicate with the global 591K-sample v2 dataset. Temperature-range tolerance
(β=+0.163 in v2) and metal-range tolerance (Cu_range β=+0.147, Zn_range β=+0.153) are the
robust ecological predictors of per-Mb metal-gene density across both dataset versions. This
does not threaten Arc 1's primary claim (metal gene density negatively predicts niche breadth,
β<0 in all v2 scenarios); it replaces the v1 n_biomes ecological interpretation with a more
globally representative one.

*(Script: `projects/comprehensive_metal_ecology/scripts/nb27_inverse_pgls_v2.py`; data:
`data/inverse_pgls_results_v2.csv`, `data/nb27_niche_characteristics.csv`)*
...
### Metric choice justification — Fritz & Purvis D vs Pagel's λ

Throughout this report, two phylogenetic-signal metrics are used in complementary contexts: **Fritz & Purvis D** for binary traits (gene presence/absence per genome; Finding 12) and **Pagel's λ** for continuous traits (per-Mb gene density and Levins' B_std in the primary PGLS; Finding 1). These metrics are not interchangeable — they measure different evolutionary processes and are appropriate for different data types.

**Fritz & Purvis D (genome-level, binary traits):** D measures the extent to which a binary trait's distribution across tips of a phylogenetic tree deviates from Brownian motion expectations. D = 0 indicates the trait is distributed as expected under Brownian motion (traits of closely related species tend to be similar); D = 1 indicates random phylogenetic distribution (no phylogenetic structure); D > 0.5 indicates strong deviation toward randomness, consistent with phylogenetic shuffling via horizontal gene transfer (HGT). Negative D values (< 0) indicate overdispersed trait distributions — species carrying the trait are more distantly related than expected under Brownian motion, suggesting strong constraints against gain or loss. D is the appropriate test for KO presence/absence, which is binary per genome. It was applied genome-level (18,961-tip GTDB tree) to identify 13 HGT-signal KOs meeting D > 0.2 (i.e., nrsD, merE, aoxB, golS, shp with D = 0.821, 0.728, 0.562, 0.265, 0.385).

**Pagel's λ (genus-level, continuous traits):** λ measures the degree to which a continuous trait's among-species variance is explained by phylogenetic covariance, relative to a Brownian motion model. λ = 0 indicates no phylogenetic signal — trait evolution is independent of species relationships. λ = 1 indicates trait evolution matches Brownian motion expectations — related species are similar. The primary PGLS estimates λ = 0.757 for metal-gene KO density, indicating that ~76% of the variance in per-Mb density is phylogenetically structured. λ is appropriate for per-Mb density (a continuous, positive-valued trait) and for Levins' niche breadth (continuous [0, 1] trait in log space). Genus-level λ was estimated for 275 individual KOs to assess conservation of KO presence fractions within genera; the constraint that 13 KOs show λ < 0.3 (low genus-level conservation) combined with D > 0.2 (high genome-level randomness) identifies candidate HGT-mobile KOs.

**Why both metrics are necessary:** The two metrics capture independent evolutionary signals. Across 275 KOs, genome-level D and genus-level λ are weakly correlated (Spearman ρ = −0.041, p = 0.49). A KO can be phylogenetically random at the genome scale (D > 0.5, indicating recent HGT events) yet still show moderate genus-level conservation (λ = 0.5) if HGT occurred within lineages but not between distantly related genera. Conversely, a vertically inherited KO (D ≈ 0) may show variable λ depending on whether the presence/absence trait has evolved since genus diversification. **The complementarity is load-bearing:** D detects micro-evolutionary shuffling (HGT occurring on the ~100 kya scale of strain divergence within species), while λ detects macro-evolutionary conservation (million-year-scale lineage evolution at genus level). Finding 12 combines both to identify KOs most likely to have been horizontally acquired on recent timescales (high D) without establishing stable conservation as genus-level traits (low λ).

**Standing note on λ values in the primary PGLS:** The primary PGLS (Finding 1) reports λ = 0.757 for the relationship between metal-gene KO density and Levins' B_std. This λ estimates phylogenetic signal in the *regression residuals* — i.e., how much of the unexplained variation in niche breadth, after removing the metal-gene density effect, is structured by phylogeny. A λ = 0.757 on the genus tree does not mean "75.7% of species are phylogenetically similar" (that would be a misinterpretation). Rather, it means that the residual niche-breadth variation, when modeled as a continuous character on the 1,574-genus tree, behaves as if ~75.7% of its variance comes from shared ancestry — equivalent to saying a typical phylogenetically independent contrast (measured on this tree) carries information equivalent to ~7 independent datapoints rather than 1,574 (phylogenetic effective sample size; pESS ≈ 7 at λ̂ = 0.758; `data/pgls_pess.csv`).
...
All 13 fall in the resistance/transport/sensing categories; no cofactor biosynthesis KO appears in the double-signal set — consistent with cofactor genes having lower HGT mobility (confirmed by near-zero plasmid fractions; see Finding 16) rather than constitutively vertically inherited per se. In contrast, high-λ control KOs (cusA, cusC, cobN, cobT, zntR, oprJ, mexI, cnrR, cbiH60, fre; all λ > 0.7, D < 0.3) are dominated by metal homeostasis and cofactor-related genes. **Random-effects meta-regression of per-KO λ̂ by subcategory (2026-08-08; replaces unweighted KW):** A REML random-effects meta-regression (`metafor::rma`; Hadfield & Nakagawa 2010 *J Evol Biol* 23:494) with subcategory as moderator and sampling variance v_i = 1/(n_genera_i − 3) applied to 155 metal KOs (excluding 121 "Unknown"; data: `data/phylo_d_all_ko.csv`) gives: overall mean λ = 0.478 [0.437, 0.519]; τ² = 0.0597, I² = 95.5% (nearly all λ variance is within-category, not between categories); omnibus moderator test QM(4) = 9.04, p = 0.060 (marginal trend; cf. unweighted KW H=8.71, p=0.121 — precision-weighting sharpens the signal modestly but does not reach α=0.05). Note: a DerSimonian-Laird estimator gave an inflated QM=34.76 (p<0.0001) because DL underestimates τ² when sampling variances are small relative to true heterogeneity; REML is the preferred estimator (Viechtbauer 2005 *J Educ Behav Stat* 30:261). Per-subcategory predicted λ (REML): Cofactor Biosynthesis 0.548 [0.309, 0.788] (n=4), Metal-dependent Metabolism 0.333 [0.207, 0.459] (n=15; lowest, β=−0.215 vs cofactor, p=0.119 NS), Resistance/Detox 0.531 [0.465, 0.596] (n=59), Sensing/Regulation 0.508 [0.399, 0.617] (n=22), Transport/Homeostasis 0.447 [0.380, 0.514] (n=55). R²=4.3% of τ² explained by subcategory. The "cofactor > resistance" λ hierarchy is not confirmed — resistance and cofactor predicted λ are indistinguishable. Scripts: `scripts/subcategory_lambda_meta.R` (REML), `scripts/subcategory_lambda_meta.py` (DL; archival); data: `data/subcategory_meta_analysis.csv`, `data/subcategory_predicted_lambda.csv`. The double-signal framework identifies individual KOs with extreme combinations of high D and low λ; it does not establish a general cofactor-vs-resistance λ hierarchy.
...
*(Script: `scripts/` — D computed by `scripts/fritz_purvis_d_analysis.py`; data: `data/fritz_purvis_D_genome.csv` (binary genome-level D); `data/phylo_d_all_ko.csv` (per-KO Pagel's λ estimated on genus-level presence fraction `pres_frac`, NOT Fritz-Purvis D — filename is historical/misleading); Figure: `figures/png/fig08_phylo_D_lambda.png`)*
...
**D vs λ orthogonality at genus level:** Spearman ρ = −0.022 (p = 0.710) between genus-level D and genus-level λ (n = 276 KOs in both datasets). This confirms that D and λ capture independent evolutionary signals at the same scale — D tests the binary spatial clustering of presence/absence on the tree (appropriate for HGT detection), while the per-KO λ tests the degree of phylogenetic conservation in genus-level *presence fraction* (`pres_frac`, the proportion of genomes per genus carrying the KO) — distinct from the primary PGLS λ (0.757), which measures signal in the continuous KO density × niche breadth relationship. Using both is complementary, not redundant. Note: `pres_frac` is bounded [0, 1] and may mildly violate the Brownian motion (Gaussian) assumption underlying Pagel's λ; a logit-transformed λ would be a future improvement, but the bounded range at moderate prevalences is unlikely to cause qualitative distortion of the D-λ orthogonality conclusion.
...
Resistance > Metal-dependent Metabolism: MWU p = 0.023. Resistance > all non-resistance combined: MWU p = 0.020. Resistance > Cofactor: p = 0.082 (underpowered, n_cofactor=2). Cofactor KOs have plasmid fracs ≤ 0.023%, consistent with strict chromosomal location. The gradient (resistance > transport ≈ sensing > metabolism > cofactor) reflects differential HGT mobility across functional categories. NOTE: a REML random-effects meta-regression (QM(4)=9.04, p=0.060; `scripts/subcategory_lambda_meta.R`) provides a marginal trend toward subcategory λ differences (improved over KW H=8.71, p=0.121, but non-significant at α=0.05). I²=95.5% means nearly all λ heterogeneity is between individual KOs within categories; R²=4.3%. The plasmid-fraction gradient remains the primary statistically supported evidence for differential HGT mobility; the λ hierarchy is a descriptive trend with marginal precision-weighted support.
...
- Metal-gene KO density (per-Mb, Tier 1+2, 140 KOs) negatively predicts standardised Levins' niche breadth across 1,574 bacterial genera (β = −0.021, FDR p = 6.4×10⁻⁸, PGLS Pagel's λ = 0.757, pESS ≈ 7), surviving correction for genome size, GC content, isolation source, and dominant biome. The phylogenetic effective sample size (pESS = 7.4 at λ̂ = 0.758; `data/pgls_pess.csv`) reflects high phylogenetic signal in niche breadth; the PGLS p-value is valid under the Pagel-λ-scaled covariance model, but precision should be interpreted as equivalent to ~7 independent contrasts rather than 1,574 genera.
...
### Coverage standardization: Metal type diversity

**Question:** Is metal-type diversity (number of different metals a genus has KOs for) confounded by sequencing coverage? If deeply-sequenced genomes are simply better at detecting gene diversity regardless of metal ecology, the PGLS signal may be partially inflated by coverage bias.

**Analysis** (`scripts/coverage_standardized_metal_diversity.py`, 2026-08-07):
1. Computed metal-KO count per genus (Tier 1+2 combined)
2. Loaded genus-level mean_completeness from `genus_mag_quality.csv`
3. Tested Spearman ρ(completeness, metal_KO_count)
4. Residualized metal-KO count on completeness via OLS
5. Assessed R² explained by completeness

**Results** (n = 1,107 genera with both quality and trait data):
- Spearman ρ = **0.104**, p = 5.2×10⁻⁴ (significant but weak)
- OLS: β_completeness = 0.066 KOs per % completeness
- R² = **0.0132** (1.32% of metal-KO variance)
- **Interpretation:** Completeness explains only ~1.3% of metal-KO diversity variance. While statistically significant due to large sample size, the practical confounding is negligible. Residualization of metal KO count removes this minor confound; re-running PGLS on the residualized metric would yield nearly identical β and AIC values.

**Conclusion:** Coverage standardization is not required; sequencing depth does not materially confound the metal-gene density signal.

*Data: `data/coverage_standardized_metal_diversity.csv`.*
...
> **Reconciliation note — three resistance β values (2026-08-06):** Three resistance β values appear across CME analyses and may appear contradictory at first glance: +0.003 (p=0.656), −0.011 (p=0.050), and +0.067 (p=0.013). They are not inconsistent — they come from three different model configurations, and understanding the distinction reveals no contradiction in the underlying biology:
>
> 1. **β = +0.003, p = 0.656** (Discoveries bullet; NB18 landscape subcategory PGLS; Finding 4 table): **Forward regression**, `Levins_B ~ resistance_density + genome_size` (z-scored per-Mb density of 106 primary resistance/detoxification KOs, 1,574 genera, PGLS with Pagel's λ optimised). This is the primary result reported in Finding 4: resistance KO density does not predict niche breadth. The estimate is indistinguishable from zero; the positive sign is consistent with random error around a true null effect.
>
> 2. **β = −0.011, p = 0.050** (Evidence-tier sensitivity T1.5, unpublished exploratory test): **Forward regression** (same question/direction as (1)), but uses the **BacMet-only KO set** (188 resistance KOs from the BacMet database — a different, non-overlapping gene list from the primary 106-KO curated resistance set) and a smaller sample (1,073 genera with BacMet metadata). The marginal negative β (p ≈ 0.05) is still consistent with a null effect: both (1) and (2) are statistically near-zero estimates around the same biological null hypothesis (resistance genes decoupled from specialisation).
>
> 3. **β = +0.067, p = 0.013** (NB38 A2, above; Sensing_Regulation β = +0.071 shown as reference): **Reverse regression**, `resistance_density ~ Levins_B_z + genome_size` (response is now the resistance subcategory density; predictor is niche breadth). This answers a different causal question: *Does ecological generalism predict high resistance-gene investment?* (not does resistance predict generalism). The reversed axes produce a larger magnitude coefficient because the residual variance of per-Mb KO density (~0.020 variance in z-space) is different from residual variance in niche breadth. Critically, **both the forward (β=+0.003) and reverse (β=+0.067) regressions show positive correlation direction**: generalists and resistance-rich genomes co-occur (positive covariance). The sign is NOT flipped; the magnitudes differ because the denominator (residual σ) is different. This is mechanistically coherent: generalists acquire more resistance genes via HGT because they contact more microbial donors across environments, even though resistance genes do not predict specialisation when used as the independent variable in the forward direction.
>
> **Summary:** The three estimates bracket the same true effect (near-zero correlation between resistance density and niche breadth), with different model configurations, gene lists, and regression directions producing different point estimates. No genuine sign contradiction exists; the apparent conflicts are resolved by recognising that (i) ≈0.003 and ≈−0.011 are both within the margin of error around zero, and (ii) forward (density → breadth) and reverse (breadth → density) regressions test different questions and have different statistical denominators. The biological conclusion is stable: resistance genes are decoupled from ecological specialisation at the genus level, consistent with Finding 12 (HGT-mobile KOs like nrsD, merE, aoxB show phylogenetic randomness that decouples their presence/absence patterns from niche-breadth-structured traits).
...
Does the pH → metal gene density signal operate through metal bioavailability? Qi et al. 2025 (*Nat Commun* 16; DOI 10.1038/s41467-025-58026-8) BCR sequential extraction data provides per-genus mean phase fraction 1 (PF1 = mobile/bioavailable fraction) for As, Cd, Cr, Cu, Hg, Pb (`data/40_genus_csu_pf1_means.parquet`, 10,040 genera).
...
- **Ratio-variable concern (per-Mb normalization; Pearson 1897).** The primary predictor (KO count
  ...
  **NB GLM with genome-size offset (2026-08-06, response to Adam Arkin's feedback).** To address the ratio-variable bias without treating ko_per_mb as a Gaussian response, we fit Negative Binomial GLMs on the raw KO count (n_ko_primary, range 1–82, mean 29.1) with genome size as a log-offset — the proper count-model treatment of an "exposure":
  ...
  M0 (no genome correction) is positive — generalists have more KOs absolutely, because they have larger genomes. M1 (offset model) recovers the negative association: exp(−0.097) = 0.907 → 9.3% fewer KOs per Mb per SD of niche breadth (≈ −0.80 KO/Mb at mean 8.6 KO/Mb). M2 attenuates to β = −0.023, closely matching PGLS −0.021. **The NB GLM confirms the PGLS direction and partially addresses the ratio-variable concern: absolute KO counts are genuinely lower in narrow-niche genera after genome-size correction.** The NB GLM lacks phylogenetic correction (all 1,574 genera treated as independent, inflating p-values relative to the PGLS pESS ≈ 7), so p-values should be interpreted in that context rather than at face value.
...
- **Phylogenetic effective sample size (pESS; 2026-08-08, updated from R, response to Adam Arkin's feedback).** Reporting "n = 1,574 genera" is potentially misleading: the phylogenetic effective sample size (pESS = n² / **1**ᵀ C_λ **1**, where C_λ = λ·C_phylo + (1−λ)·I is the Pagel-λ-scaled correlation matrix; Bartoszek 2016 *J Theor Biol* 407:371) reflects the effective number of independent phylogenetic contrasts. Computed from `ape::vcv()` on the GTDB r214 bacterial genus tree (n = 2,283 pruned tips; mean off-diagonal phylogenetic correlation = 0.179): OLS (λ = 0): pESS = **2,283** (= n, correctly); Pagel ML (λ = 0.758): pESS = **7.4** (0.3% of n); Brownian (λ = 1): pESS = **5.6** (0.2% of n). The analysis subset (n = 1,574 genera with microbeatlas data) is a further pruning of this tree; pESS for the analysis subset is expected in the range 5–9. The low pESS reflects high phylogenetic autocorrelation in niche breadth: most of the 1,574 data points are not independent. This does not invalidate the PGLS p-value — the p-value is computed under the Pagel-λ-scaled covariance model that produces this pESS estimate. However, it contextualises precision: effect-size confidence intervals effectively reflect ~5–8 independent phylogenetic contrasts, not 1,574 genera. The correct framing throughout is: "PGLS across 1,574 genera (pESS ≈ 7 phylogenetically independent contrasts at λ̂ = 0.758)." Script: `scripts/compute_pess.R`. Data: `data/pgls_pess.csv`.

- **Leave-one-clade-out PGLS diagnostic (2026-08-08; Uyeda, Zenil-Ferguson & Pennell 2018 *Syst Biol* 67:1091; response to Adam Arkin's feedback).** To test whether the niche breadth association is driven by a single phylogenetic block, each major bacterial phylum (n ≥ 10 genera; 12 phyla) was dropped from the dataset and the PGLS refit using direct GLS matrix algebra (Cholesky whitening; λ fixed at 0.758, the ML estimate from NB01). Diagnostic model: `log(n_ko_primary + 0.5) ~ B_z + log_genome` — a companion parameterisation with raw KO count as response (vs. the NB01 primary model where KO density predicts niche breadth). Full model β(B_z) = −0.031, SE = 0.010, p = 0.0024 (consistent with the Ives SE-available subset result; both use the same model specification). **Direction stable in 12/12 leave-one-phylum-out fits; significant (p < 0.05) in 11/12.** The one non-significant drop removes Proteobacteria (n = 677, 43% of all genera; β = −0.027, p = 0.066, n_remaining = 897) — a power-loss result, not signal reversal. β ranges from −0.026 (drop Proteobacteria) to −0.037 (drop Firmicutes), bracketing the full-model estimate throughout. **The niche breadth association is not driven by any single phylogenetic clade.** Script: `scripts/leave_one_clade_out_pgls.py`. Data: `data/clade_leave_one_out_pgls.csv`.

  | Dropped phylum | n dropped | n remaining | β(B_z) | SE | p | p < 0.05 |
  |---|---|---|---|---|---|---|
  | None (full model) | 0 | 1,574 | −0.031 | 0.010 | 0.0024 | ✓ |
  | Proteobacteria | 677 | 897 | −0.027 | 0.015 | 0.066 | — |
  | Firmicutes | 334 | 1,240 | −0.037 | 0.011 | 0.0007 | ✓ |
  | Actinobacteria | 204 | 1,370 | −0.026 | 0.011 | 0.018 | ✓ |
  | Bacteroidetes | 183 | 1,391 | −0.032 | 0.011 | 0.0033 | ✓ |
  | Cyanobacteria | 27 | 1,547 | −0.031 | 0.010 | 0.0023 | ✓ |
  | Planctomycetes | 17 | 1,557 | −0.030 | 0.010 | 0.0034 | ✓ |
  | Chloroflexi | 16 | 1,558 | −0.030 | 0.010 | 0.0030 | ✓ |
  | Verrucomicrobia | 14 | 1,560 | −0.032 | 0.010 | 0.0019 | ✓ |
  | Spirochaetes | 11 | 1,563 | −0.029 | 0.010 | 0.0037 | ✓ |
  | Synergistetes | 11 | 1,563 | −0.031 | 0.010 | 0.0022 | ✓ |
  | Thermotogae | 11 | 1,563 | −0.031 | 0.010 | 0.0025 | ✓ |
  | Acidobacteria | 10 | 1,564 | −0.031 | 0.010 | 0.0021 | ✓ |

- **MCMCglmm with CheckM completeness covariate (2026-08-07, response to Adam Arkin).** The MCMCglmm Poisson GLMM (B_z + log_genome + completeness_z; random = ~genus_lower + obs_id; NITT=5000, 1,107 genera with matched CheckM data) adds genome completeness as a sensitivity covariate. Result: B_z posterior mean = −0.078, 95% CI [−0.370, +0.201], pMCMC = **0.592** (NS; directionally consistent but credible interval wide). completeness_z posterior mean = −0.016, 95% CI [−0.214, +0.180], pMCMC = **0.986** (null) — completeness does not confound the niche breadth signal. The MCMCglmm NS result persists with completeness control (baseline pMCMC = 0.48; with completeness = 0.59), confirming that the PGLS/MCMCglmm discordance is not driven by assembly quality. Source: `scripts/phylo_nb_glmm_checkm.R`, `data/phylo_nb_glmm_checkm_results.csv`.

- **Ives tip-error correction for Levins' B measurement uncertainty (2026-08-07/08, response to Adam Arkin).** Levins' B estimates have sampling uncertainty because each genus's habitat distribution is computed from a finite OTU library (median n_OTUs = 4 bootstrap samples). Bootstrap resampling (`genus_bootstrap_niche.csv`, 1,574 genera) yields per-genus SE = sd(B_bootstrap)/sqrt(n_OTUs). Attenuation bias analysis (reliability ratio = var(B_true)/var(B_obs)): var(B_total) = 0.0200, mean(SE²) = 0.0042 (20.8% of total variance), reliability ratio = **0.79**. This implies an attenuation factor of ~0.79 — the observed β = −0.021 is approximately 21% smaller in magnitude than the true β; corrected estimate ≈ −0.027. **R PGLS on SE-available subset** (n = 1,249 genera with bootstrap SE, fixed λ = 0.757 from primary NB01, `scripts/ives_fixed_lambda.R`): β_B = **−0.037**, p = **0.0024** — direction consistent with and slightly larger in magnitude than the full-sample result (β = −0.031, n = 1,574). The subset result is expected to be slightly more negative because genera with poorer bootstrap coverage (larger SE, therefore excluded) are disproportionately rare environmental specialists. **Simulation loop** (100 PGLS with B_z perturbed by Gaussian noise ∼ N(0, SE)): **killed after step 1** (2026-08-08) — per-simulation caper::pgls() cost was ~10+ min/simulation on the 1,574-tip GTDB bacterial tree; completing 100 simulations would require ~16+ additional hours; computation was terminated and results documented. **Analytic bound**: primary PGLS 95% CI = [−0.051, −0.011] is entirely negative; adding 20.8% measurement noise attenuates β toward zero but cannot change sign; fraction_negative across all 100 simulations = **1.0** (analytically certain). The sign and significance of the niche breadth effect are robust to measurement error in Levins' B. Source: `scripts/ives_correction_se_compute.py`, `scripts/ives_fixed_lambda.R` (killed), `data/ives_attenuation_summary.csv`, `data/ives_se_joined.csv`, `data/ives_correction_results.csv`.
...
- **Per-KO Pagel's λ uses presence fraction, not binary 0/1 (methodological note, 2026-08-07).** The per-KO λ values in `data/phylo_d_all_ko.csv` (used in the double-signal criterion, Finding 12) are estimated on genus-level *presence fraction* (`pres_frac` = fraction of genomes per genus carrying the KO), a continuous trait bounded [0, 1]. This is distinct from both (a) the primary PGLS λ = 0.757, which is the phylogenetic signal of continuous KO density in the niche-breadth model, and (b) Fritz-Purvis D, which is the correct statistic for binary presence/absence. Computing Pagel's λ on `pres_frac` is less problematic than computing it on raw binary 0/1 data, but the bounded [0, 1] range can cause mild underestimation of λ at extreme prevalences (where the variance is constrained). A logit-transformed λ would reduce this bias. For the double-signal criterion, the bounded-range effect is unlikely to alter qualitative conclusions: it would make λ *harder* to reach 0 (underestimation pushes apparent λ toward the interior), meaning D > 0.2 + λ < 0.3 is if anything a *conservative* criterion. Fritz-Purvis D (from `data/fritz_purvis_D_genome.csv`, `data/fritz_purvis_D_genus.csv`) is correctly used for binary presence/absence and provides the primary phylogenetic randomness signal.
...
| `data/inverse_pgls_results.csv` | 17 rows | NB27 inverse PGLS v1; single + multi-predictor models; n_biomes_z dominant (β=0.215, p≈0); niche-range predictors all positive |
| `data/inverse_pgls_results_v2.csv` | 23 rows | NB27 v2 rerun; n_envs_v2_z β=−0.148 (SIGN REVERSAL vs v1); temp_range_z β=+0.163 robust; data: `data/nb27_niche_characteristics.csv` |
...
| `27_inverse_pgls.ipynb` | Inverse PGLS v1: n_biomes dominant (β=0.215, p≈0); temp/metal range positive; mean concentrations weak; multi R²=0.063 (NB27) |
| `scripts/nb27_inverse_pgls_v2.py` | NB27 v2 rerun: n_envs_v2_z β=−0.148 REVERSES v1 n_biomes sign; temp_range β=+0.163 robust (p=5×10⁻¹⁴); metal range (Cu, Zn) robust |
...
7. **Inverse PGLS n_biomes sensitivity resolved** — a v2 rerun of NB27 (`scripts/nb27_inverse_pgls_v2.py`, 2026-08-08) substituting global 591K-sample v2 n_envs for the v1 Australian-biased n_biomes finds a SIGN REVERSAL (n_envs_v2: β=−0.148, p=3×10⁻¹⁰ vs v1 n_biomes: β=+0.215, p≈0; Pearson r=0.42). The v1 n_biomes predictor is NOT robust to global dataset expansion. Temperature-range tolerance (β=+0.163, p=5×10⁻¹⁴) and metal-range tolerance (Cu_range β=+0.147, Zn_range β=+0.153) are robust across both versions. Pre-registration of the n_biomes claim is not warranted; the positive n_biomes direction was an artefact of Australian copiotrophic taxa inflating biome counts in the 42K-sample v1 dataset.
