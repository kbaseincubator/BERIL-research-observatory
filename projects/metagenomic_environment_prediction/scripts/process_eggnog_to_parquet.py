...
Outputs:
  - data/ko_annotations/{sample_id}.parquet  -- per-sample intermediates (resumable)
  - data/mag_ko_annotations.parquet          -- unified long-format table

Usage:
    python scripts/process_eggnog_to_parquet.py [--max-samples N]
    python scripts/process_eggnog_to_parquet.py --merge-only
"""
...
KO_ANNOT_DIR = DATA_DIR / 'ko_annotations'
KO_ANNOT_DIR.mkdir(parents=True, exist_ok=True)
...
_META_CACHE = DATA_DIR / 'soil_mag_metadata_cache.csv'


def get_mag_metadata():
    """Return soil MAG metadata. Uses a CSV cache to avoid Spark on restarts."""
...
def load_contig_map(soil_mag_ids: set):
    """Load contig->MAG mapping; return (polars DataFrame, Python frozenset of contigs).

    The frozenset is used for O(1) membership checks during line-by-line decompression
    so we only pass soil-MAG rows to polars, avoiding 4 GB+ Rust buffers for large samples.
    """
    parquet_path = DATA_DIR / 'mag_contig_map.parquet'
    print(f"Loading contig->MAG map (filtering to {len(soil_mag_ids):,} soil MAGs)...")
    t0 = time.time()
    df = pd.read_parquet(parquet_path, columns=['mag_id', 'contig'])
    df = df[df['mag_id'].isin(soil_mag_ids)].drop_duplicates(subset='contig').copy()
...
def merge_parquets():
    ...
    Layout: data/mag_ko_annotations/sample_id={SID}/data.parquet
    ...
    files = sorted(KO_ANNOT_DIR.glob('*.parquet'))
    if not files:
...
def _process_sample(sid, fetch_fn, contig_to_mag_pl: pl.DataFrame,
                    contig_set: frozenset, soil_mag_ids: set):
    """Download + parse one sample's eggnog; aggregate per (mag_id, ko_id); save parquet.

    Memory strategy: decompress line-by-line and keep only soil-MAG rows before
    passing to polars. Large SAMEA samples can be 476 MB gzip = 4 GB decompressed
    with millions of rows from non-soil MAGs. By filtering during decompression,
    polars only receives the ~10% of rows belonging to soil MAGs (~400 MB instead
    of 4 GB), keeping per-worker peak at ~500 MB instead of ~4 GB.

    Memory per worker: 36 MB (gzip) + 400 MB (filtered buf) + 200 MB (polars) = ~640 MB
    5 workers: ~3.2 GB extra; total pod ~15 GB — well under 24 GB limit.
    """
    out_path = KO_ANNOT_DIR / f"{sid}.parquet"
    if out_path.exists():
        return "cached"
    ...
    # Decompress line-by-line; filter to soil-MAG rows using precomputed frozenset
    col_names = None
    filtered_lines = []
    try:
    ...
    # Parse only the soil-MAG rows with polars (fast Rust CSV parser)
    ...
    # Inner join = filter to soil MAGs + lookup mag_id in one vectorized pass
...
def main():
    ...
    soil_mag_ids = set(mag_meta_df['mag_id'])
    contig_to_mag_pl, contig_set = load_contig_map(soil_mag_ids)

    sample_groups = mag_meta_df.groupby('sample_id')['mag_id'].apply(list).to_dict()
    done_ids = {p.stem for p in KO_ANNOT_DIR.glob('*.parquet')}
    ...
    def _worker(sid_mids):
        ...
        outcome = _process_sample(sid, client.fetch_eggnog_bytes,
                                  contig_to_mag_pl, contig_set, soil_mag_ids)
        elapsed = time.time() - t0
        with _lock:
