...
The paired adversarial review on 2026-04-24 (see `REVIEW_2_ADVERSARIAL.md`) identified five issues that were partially addressed in Phase 2b. **Three were closed on 2026-04-25**: item 17 (C4 — per-species OG test) using the `top50_og_species.csv` matrix that NB03 already cached — 48/50 OGs survive real per-species phylum+genome-size regression at q<0.05 (see §11 "Genome-size covariate"); item 19 (full 65-species subclade scan) — 5/17 testable species pass Bonferroni-Fisher (3 also Cochran-valid), but 47/65 candidate species lack phylo tree coverage in BERDL (see §11 "Subclade genome ID fix" and `docs/pitfalls.md`); and item 21 (bakta-vs-IPS Pfam audit) — 12/22 marker Pfams found systematically missing from `bakta_pfam_domains`, project unaffected because NB10 uses IPS, pitfall documented for future projects (see §11 "Pfam recovery" and `docs/pitfalls.md`). The remaining two items require deeper work and are listed here so that follow-up milestones can schedule them explicitly.
