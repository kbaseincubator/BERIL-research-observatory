...
The paired adversarial review on 2026-04-24 (see `REVIEW_2_ADVERSARIAL.md`) identified five issues that were partially addressed in Phase 2b. One (C4 — per-species OG test) was **closed on 2026-04-25** using the `top50_og_species.csv` matrix that NB03 already cached (see §11 "Genome-size covariate" above — 48/50 OGs survive real per-species phylum+genome-size regression at q<0.05). The remaining four items require deeper work and are listed here so that follow-up milestones can schedule them explicitly.
