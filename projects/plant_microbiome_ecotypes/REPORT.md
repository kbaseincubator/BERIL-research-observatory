...
**Genome-size covariate for novel OGs (C4, NB14) — two components, one solid, one a simulation placeholder**: This deferred control has two parts and the paired adversarial review correctly flagged the limits of each.

*(a) Dual-nature rate by genome-size quartile (solid)*: Among plant-associated species, the dual-nature rate is **54% in the bottom genome-size quartile vs 87% in the top three quartiles** (measured directly on refined cohort data without simulation). This reaffirms the NB08 finding that ~33% of dual-nature species shift to neutral when genome size is normalized — the headline 78.7% dual-nature rate masks a real genome-size gradient, and the gradient itself is observational, not model-dependent.

*(b) Per-OG logistic regression (prevalence-based simulation, NOT per-species test)*: The "50 of 50 novel OGs retain a positive plant-association coefficient after phylum + log₁₀(genome_size) control" result is produced by drawing per-species OG presence from `Bernoulli(prev_plant)` for plant species and `Bernoulli(prev_nonplant)` for non-plant species, and regressing that synthetic outcome on the covariates. Because genome size never enters the data-generating process, the simulation **cannot fail** to recover a positive `is_plant` coefficient whenever `prev_plant > prev_nonplant`, which is true by construction for every OG in the novel set (the OGs were selected precisely for that prevalence gap). The "50/50" number is therefore a sanity check on the regression code, **not** evidence that the OGs survive genome-size confounding when tested on their actual per-species presence vectors. C4 is therefore **not fully addressed** by NB14 Cell 3. A real test requires pulling the per-species OG presence matrix from the server-side `eggnog_mapper_annotations` table (as NB03 did in aggregate) and regressing that actual vector on `is_plant + log₁₀(genome_size) + phylum` — this is deferred to follow-up. What we can say now, with high confidence from NB09's functional annotation, is that the 50 OGs are cytochrome oxidase subunits, Fe-S cluster biosynthesis, inorganic ion transport, and energy metabolism genes — functions not obviously scaling with genome size; but confirming that statistically requires the per-species pull. *(Notebook: 14_deferred_controls.ipynb Cell 3, data/genome_size_control.csv. The simulation nature of 50/50 was clarified 2026-04-24 after paired adversarial review; see §Limitations for the remaining work.)*
...
**Pfam recovery (Pfam gap, NB13 Cell 3)**: The Phase 1 NB02 query `WHERE pfam_id IN ('PF00771', …)` returned zero rows because `bakta_pfam_domains` stores versioned IDs (`PF00771.22`). Re-running with `LIKE 'PF00771%'` patterns recovered **19,364 domain hits across 7,962 species**. Five of ten queried Pfam IDs produced hits: nitrogenase NifH PF00142 (6,632 hits, 4,040 species, 32.4% core), cellulase GH5 PF00150 (6,898 / 3,234 / 25.9%), pectate lyase 3 PF12708 (4,149 / 2,283 / 24.6%), pectate lyase PF00544 (1,115 / 546 / 20.8%), and T6SS VgrG PF05943 (570 / 343 / 10.9%). The remaining five Pfam IDs returned 0 hits even with the LIKE query, but for two different reasons that the initial Phase 2b write-up conflated and that the paired adversarial review flagged. T3SS inner rod **PF00771** (18,598 hits in InterProScan) and T3SS PrgH **PF01313** (13,576 hits in InterProScan) *do* exist abundantly in the pangenome but are silently omitted from `bakta_pfam_domains` — a data-source artifact likely reflecting the bakta build's reduced Pfam HMM profile set. This means the Phase 1 NB02 query missed these T3SS domains not because they were absent but because the bakta annotation pipeline does not emit them; the InterProScan table in the same database does. Secretin PF01514, VirB8 T4SS PF03135, and T6SS Hcp PF05936 are absent from both `bakta_pfam_domains` and `interproscan_domains` in this pangenome build. The practical downstream implication is that the NB10 refined cohort pipeline — which already uses `interproscan_domains` for T3SS-related marker detection — was not undercounting T3SS; the Pfam LIKE recovery confirms rather than expands the InterProScan numbers for T3SS specifically. For the CWDEs and NifH, the bakta recovery genuinely adds marker coverage beyond InterProScan alone. **Impact on cohort assignments**: 4,217 species gain at least one new marker — 2,872 gain nitrogen fixation, 1,082 gain CWDE pectinase, 811 gain CWDE cellulase. The higher core-genome fractions of NifH (32.4%) vs the CWDEs (21–26%) are consistent with the H2 pattern. *(Notebook: 13_validation_pfam_subclade.ipynb, data/pfam_recovery_hits.csv, data/pfam_recovery_impact.csv; see docs/pitfalls.md for the bakta-vs-IPS data-source gap.)*
...
| C4 | Genome-size confound | Partial fix | NB14 — quartile analysis of dual-nature rate (54% bottom Q vs 87% top) is solid; the 50/50-OG result is a prevalence-based simulation, not a per-species test (real per-species test deferred) |
...
| Pfam | Versioned-ID format | Fixed (+ new gotcha) | docs/pitfalls.md, NB13 Cell 3. Secondary finding: `bakta_pfam_domains` silently omits PF00771/PF01313 that InterProScan finds in 18k+ gene clusters — data-source gap, not domain absence |
...
The paired adversarial review on 2026-04-24 (see `REVIEW_2_ADVERSARIAL.md`) identified five issues that were partially addressed in Phase 2b but require deeper work to close. They are listed here so that follow-up milestones can schedule them explicitly.

17. **Per-species OG matrix for genuine genome-size control** (C4, NB14 Cell 3): The 50/50 OGs surviving phylum + genome-size control is produced by simulating OG presence from prevalence rates, not the real per-species OG vector. Because genome size never enters the data-generating process, the result is mechanically predetermined and cannot fail — it is a sanity check on the regression code, not a test. Closing this requires pulling the per-species OG presence matrix from `eggnog_mapper_annotations` (server-side; NB03 already aggregated it at the species-OG level), and re-regressing each of the 50 OGs on `is_plant + log₁₀(genome_size) + phylum`. Expected work: 1–2 hours of Spark + local compute.
...
21. **Pfam data-source gap between bakta and InterProScan** (new finding from NB13 Pfam recovery): `bakta_pfam_domains` silently omits ≥18K gene clusters that `interproscan_domains` annotates with the same Pfam accession (PF00771 T3SS inner rod and PF01313 T3SS PrgH both confirmed). Closing this as a standing practice requires a project-wide audit of every Pfam query that hit `bakta_pfam_domains` to cross-check against `interproscan_domains`; NB10's pipeline already uses InterProScan for T3SS so the NB10 results stand, but any future bakta-based Pfam query should paired-check.
