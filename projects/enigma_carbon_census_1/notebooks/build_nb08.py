...
cells = [
    ...
    ("md", "## The discovery funnel (83 → callable)\n"
           ...
           "**Callable semantics (review I1).** A compound is *callable* if there is direct evidence "
           "an organism can use it as a carbon source. Two non-overlapping bases qualify:\n"
           "- `enigma_isolate_call` — NB04 mapped a knowledge-derived utilization signature to an "
           "ENIGMA isolate (`tier != T0_organism_dark`).\n"
           "- `measured_fitness` — the compound has a **Tier-1 measured** RB-TnSeq carbon-source "
           "fitness experiment in the Fitness Browser (`best_tier == 'T1_measured'`), confirmed "
           "structurally by the InChIKey re-match in NB02c. Measured growth-on-carbon is the "
           "strongest possible evidence and must not be scored dark just because the organism-"
           "mapping path returned only biosynthetic-direction signatures.\n\n"
           "This correction adds exactly one compound — **lauric acid** — on a `measured_fitness` "
           "basis (8 → 9 callable). Its Fitness Browser organism is a reference bacterium, **not** "
           "an ENIGMA isolate, so deliverable (a) (the ENIGMA-isolate table) is unchanged."),
