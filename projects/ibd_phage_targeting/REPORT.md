...
## Thesis (Pillars 1–4 closed; Pillar 5 opened)
...
**Pillar 5 per-patient cocktail framework** (NB15): 23 UC Davis CD patients × per-patient profile (ecotype + Montreal + calprotectin + medication + Kuehl_WGS Tier-A presence) → **14 of 23 patients (61 %) receive concrete phage cocktail drafts**; all 9 E1 patients carry the full Tier-A pathobiont module. **Pure phage cocktail is NOT feasible for any E1 patient** — the per-patient cocktail must be a **3-strategy hybrid**: (1) direct phage targeting (E. coli AIEC + E. bolteae PMBT24 + E. lenta PMBT5), (2) alternative therapies (H. hathewayi GAG-degrading enzyme inhibitors; F. plautii BA-binding co-therapy), (3) limited / engineered (M. gnavus lytic-locked phage engineering or biochemical glucorhamnan-synthesis target). ***F. plautii* BA-coupling cost is the dominant E1 design constraint** (present in 78 % of patients including all 9 E1; HIGHEST BA-coupling cost from NB09c §13 + Pillar-4 GAP from NB12 = double penalty → deprioritize from cocktail). ***E. coli* present in only 35 % of UC Davis patients** — much lower carriage than expected for AIEC-canonical species; the NB13 5-phage cocktail is directly applicable to ~8/23 patients only. Patient 6967 (longitudinal E1 ↔ E3 drift) is the central per-patient stability test for state-dependent cocktail dosing.

**Status**: Interim synthesis — **Pillar 1 closed**; **Pillar 2 fully closed**; **Pillar 3 fully closed** (12 notebooks; all 8 H3 sub-hypotheses tested + cross-cohort metabolomics replication + multi-omics joint factor pilot CC1 r=0.96); **Pillar 4 fully closed** (3 notebooks NB12/NB13/NB14; 3-layer phage-evidence stack; concrete 5-phage E. coli cocktail; gut-anaerobe coverage gap confirmed across all 3 layers); **Pillar 5 opened** (NB15: per-patient cocktail drafts for all 23 UC Davis patients; hybrid 3-strategy framework; F. plautii BA-cost as dominant constraint; patient 6967 longitudinal stability test queued). NB16 longitudinal stability + dosing strategy + NB17 cross-cutting synthesis remain.
...
NB15 opens Pillar 5 with **concrete per-patient cocktail drafts for all 23 UC Davis CD patients**, applying the Pillar 1-4 framework as a per-patient rule book.
...
| Mixed (6967) | 1 | State-dependent dosing (per-visit ecotype rebalancing) |

### What this report does not yet contain

**Pillar 5 remaining**: NB16 — per-patient longitudinal stability + dosing strategy (focused on patient 6967 E1↔E3 drift + any other multi-timepoint patients); NB17 — cross-cutting synthesis + clinical-translation roadmap. **Pillar-4 follow-ups**: external-DB queries (INPHARED ~25K phages, IMG/VR ~3M UViGs) for the 3 gut-anaerobe coverage gaps (H. hathewayi, M. gnavus lytic alternatives, F. plautii) — close the 2-of-6-actionable phage-GAP before Pillar 5 cocktail finalization.
...
**The project's central question** — can we derive ecotype-specific and per-patient pathobiont target lists for rational phage-cocktail design? — now has a project-internal answer that is robust across pathway, BGC, metabolite, strain, and serology evidence streams. After 21 notebooks (Pillars 1–3) and two adversarial reviews catching 9 critical + 16 important methodological issues, the project establishes:
...
**Pillars 1–4 closed; Pillar 5 opened.** Only NB16/NB17 + Pillar-4-followup external-DB queries remain. Updated outline:
...
- **Pillar 5 (opened with NB15)** — Per-patient profile + cocktail draft for all 23 UC Davis patients. 14 of 23 (61 %) have concrete cocktails; pure phage cocktail not feasible for E1 (must be 3-strategy hybrid); F. plautii BA-cost is dominant E1 design constraint; E. coli only in 35 % of cohort. 4 patient-design categories. Patient 6967 longitudinal stability test queued for NB16.
- **Pillar 5 remaining** — NB16 longitudinal stability + state-dependent dosing strategy (patient 6967 E1↔E3 deep dive); NB17 cross-cutting synthesis + clinical-translation roadmap.
...
5. **Per-target NB05 Tier-A annotation profile** (Novel Contribution #16, #18-driven). For each actionable Tier-A pathobiont, annotate alongside the A3–A6 score: (a) iron/AIEC-mediated vs other CD specialization mechanism (NB08a §2); (b) bile-acid coupling cost (NB09c §13); (c) species-abundance-mediated vs strain-content-mediated (NB10a §14). Converts "Tier-A score = phage target priority" into a multi-attribute decision profile for per-patient cocktail design.
6. **AIEC genome-content survey via `kbase_genomes` + `kbase_ke_pangenome`** (per plan v1.9 raw-read-deprecation). Six converging lines of evidence point to AIEC iron-acquisition + *pks*-island as the dominant CD pathobiont specialization. The natural deeper test scoped to no-raw-reads: query `kbase_genomes` for *E. coli* genomes carrying *pks* + Yersiniabactin + Enterobactin simultaneously, cross-reference with `fact_strain_competition` UHGG IDs. Gene-presence/absence query at the genome level, not per-sample strain calls.
7. **Pillar 4 — pathobiont × phage coverage matrix** (NB12–NB14). PhageFoundry `phagefoundry_strain_modelling`, `phagefoundry_ecoliphages_genomedepot`, `phagefoundry_klebsiella_*` for the 6 actionable Tier-A. Combine with bile-acid coupling cost (NB09c) to rank candidate cocktails by (a) phage availability and (b) ecological cost.
8. **Pillar 5 — UC Davis per-patient cocktail drafts** (NB15–NB17). 23 patients × ecotype × Tier-A profile → per-patient candidate cocktail. Patient 6967 longitudinal E1 ↔ E3 shift is the central per-patient stability test.

### Cross-cohort + clinical-workflow follow-ups

9. **HMP2 raw MetaPhlAn3 re-ingestion** — would (a) refit ecotypes on ≈ 11.5 K samples (potentially improving bootstrap ARI 0.13–0.17), (b) provide second-substudy E3 Tier-A meta-analysis, (c) test H1c clinical-classifier on HMP2. Currently `PENDING_HMP2_RAW` in `lineage.yaml`; cMD R-package fetch resolves the external-replication blocker (NB04h).
10. **Ecotype-defining qPCR panel design** — if 4–6 species (*F. prausnitzii*, *P. copri*, *P. vulgatus*, *B. fragilis*) carry the discriminative load, qPCR enables rapid clinical ecotype assignment. H1c demonstrated clinical-covariate-only assignment is not viable.
11. **Within-substudy × within-ecotype design as publication material** (Novel Contribution #8). The NB04e confound-free meta design is the project's most portable methodological contribution; worth writing up as a standalone methods paper.
12. **External cohort validation beyond HMP2** — additional cMD sub-studies with CD + nonIBD that populate E3 (currently only HallAB_2017 qualifies); non-Western IBD cohorts to test E2 Tier-A. MGnify MAG catalog is genome-level only (NOT sample-level — documented in `docs/pitfalls.md`).
