...
**Pillar 4 phage-targetability framework**: a 3-layer phage evidence stack (literature curation NB12 + experimental susceptibility NB13 + in-vivo phageome NB14) yields **a concrete 5-phage E. coli AIEC cocktail covering 95 % of 188 PhageFoundry-tested strains** (DIJ07_P2 + LF73_P1 + AL505_Ev3 + 55989_P2 + LF110_P2; Phapecoctavirus + Tequatrovirus + Mosigvirus + Kayfunavirus families isolated against canonical AIEC reference strains LF82/LF73/536), and a clear **2-tier vs phage-GAP stratification** of the actionable Tier-A: clinical-trial-stage (E. coli EcoActive precedent) → lytic-literature (E. lenta PMBT5, E. bolteae PMBT24) → temperate-only (M. gnavus 6 phages) → coverage-GAP (H. hathewayi, F. plautii). The 2 phage-GAP species are also the highest-NB05-scored (4.0, 3.3) and require external DB queries (INPHARED/IMG/VR) before Pillar 5 cocktail finalization. **For per-patient cocktail design (Pillar 5): targeting the actionable Tier-A pathobiont module is mechanistically supported, ecotype-stratified, and phage-availability-stratified, with target priority modulated by bile-acid coupling cost + species-abundance-vs-strain-content mediation profile**.

**Status**: Interim synthesis — **Pillar 1 closed**; **Pillar 2 fully closed**; **Pillar 3 fully closed** (12 notebooks; all 8 H3 sub-hypotheses tested + cross-cohort metabolomics replication + multi-omics joint factor pilot CC1 r=0.96); **Pillar 4 fully closed** (3 notebooks NB12/NB13/NB14; 3 complementary phage-evidence layers — curated literature + PhageFoundry experimental susceptibility 96 phages × 188 strains × 17,672 pairs + HMP2 in-vivo phageome 630 samples; concrete 5-phage E. coli cocktail at 95 % strain coverage; gut-anaerobe Tier-A coverage gap confirmed across all 3 layers). Pillar 5 (UC Davis 23-patient per-patient cocktails) in progress.
...
**Pillar 5** (UC Davis 23-patient per-patient cocktail drafts; NB15-NB17) lands in subsequent syntheses. **Pillar-4 follow-ups**: external-DB queries (INPHARED ~25K phages, IMG/VR ~3M UViGs) for the 3 gut-anaerobe coverage gaps (H. hathewayi, M. gnavus lytic alternatives, F. plautii) — would close the 2-of-6-actionable phage-GAP before Pillar 5 cocktail finalization.
...
**Pillars 1–4 closed.** Only Pillar 5 (UC Davis per-patient cocktail drafts) + Pillar-4-follow-up external-DB queries remain. Updated outline:
...
- **Pillar 3 (fully closed)** — All 8 H3 sub-hypotheses tested with 12 notebooks (NB07a/b/d + NB07_v1.8 + NB07c + NB08a + NB09a/b/c/d + NB10a + NB11). Two cross-corroborated mechanism narratives (iron + bile-acid 7α-dehydroxylation) + cross-cohort replication on 3 themes (urobilin/acyl-carnitines/PUFAs, NB09b) + metabolite-feature ecotype stability falsified (NB09d) + multi-omics joint factor pilot (NB07d) showing all Pillar 3 narratives collapse to a single CD-vs-nonIBD canonical axis at r=0.96.
- **Pillar 4 (closed)** — Three notebooks NB12 + NB13 + NB14 producing 3-layer phage-evidence stack: curated literature (NB12) + PhageFoundry experimental susceptibility 96 phages × 188 strains × 17,672 pairs (NB13) + HMP2 in-vivo phageome 630 samples (NB14). Concrete 5-phage E. coli AIEC cocktail at 95 % strain coverage; gut-anaerobe Tier-A coverage gap confirmed across all 3 layers (H. hathewayi + F. plautii). 3-layer convergence is the Pillar-4 rigor signal (Novel Contribution #22).
- **Pillar 5 (NB15–NB17, pending)** — UC Davis medication-class harmonization, per-patient pathobiont ranking using NB05 Tier-A × Pillar-3 mechanism profile × Pillar-4 phage-availability stratification → per-patient cocktail draft, longitudinal stability check (patient 6967's E1 ↔ E3 shift), cross-cutting synthesis.
