...
- **v1.9** (2026-04-25): **All raw-read-dependent follow-ups deprecated permanently.** After NB07c + NB08a closed the iron-acquisition narrative with five converging evidence streams, surveyed every "deferred to future work" / "natural next test" item in REPORT.md and RESEARCH_PLAN.md and explicitly dropped any path that requires re-running MetaPhlAn3 / HUMAnN3 / StrainPhlAn / inStrain / antiSMASH on raw FASTQ that we do not have. The project's data scope is fixed at: precomputed mart tables (`fact_*`, `ref_*`) + curatedMetagenomicData R-package fetches + BERDL precomputed collections (`kbase_genomes`, `kbase_ke_pangenome`, `kescience_*`, `phagefoundry_*`). Specific deprecations:
  - **NB10 strain-resolution AIEC test (raw reads path) DROPPED**. Was Future Direction #9 in REPORT.md. Reframed: the *only* strain-level data in this project's scope is `fact_strain_competition` (Kumbhari/LSS-PRISM cohort, 15,520 sample × strain rows by UHGG genome ID) → NB10a is feasible (Kumbhari-only, no raw reads). Cross-cohort AIEC strain-resolution on cMD is **not feasible without raw reads** and is dropped.
  - **Alternative AIEC strain-resolution path** (no raw reads): query `kbase_genomes` for *E. coli* genomes carrying the *pks* island (clbA-clbS gene cluster) + Yersiniabactin BGC + Enterobactin BGC; cross-reference AIEC strain prevalence against `fact_strain_competition` UHGG IDs where Kumbhari has *E. coli* coverage. This is a **gene-presence/absence query at the genome level**, not a sample-level strain-call from reads. Promoted to Future Direction in REPORT.md.
  - **NB08b UC-Davis per-patient BGC annotation DROPPED.** Would need UC Davis raw reads + antiSMASH; we have only Kuehl_WGS Kaiju taxonomic calls in the mart. Per-patient BGC presence would have to be inferred from species-level BGC catalog matches (NB08a §1 Tier-A repertoire), which is a coarser proxy and is already implicitly available. Pillar 5 cocktail-design hand-off should use NB05 + NB06 + NB08a §2 directly.
  - **Strict H3c interaction-term test DROPPED.** Would need species-stratified per-sample BGC abundance (raw HUMAnN3 `*_genefamilies.tsv` × species × sample); not in mart. NB08a's main-effect-only design is the operational test for this project. Limitation already documented.
  - **HMP2 raw HUMAnN3 reprocessing DROPPED as a follow-up.** Was historically blocked on `PENDING_HMP2_RAW`. Resolution: NB04h established that cMD R-package HMP2 MetaPhlAn3 fetch is sufficient for external ecotype + Tier-A replication; raw HMP2 reprocessing for cross-modality integration (HMP2 metabolomics × HMP2 taxonomy at sample-level) is achievable via cMD-fetch sample-key joins, *not* via raw-FASTQ reprocessing. NB09a will use this approach.
  - **HUMAnN3 species-stratified `*_genefamilies.tsv` per-pathway-attribution DROPPED.** Would need raw reads. NB08a §2 genomic BGC content provides the equivalent fine-grained iron-pathway attribution at the gene-cluster level instead.
  - **Items KEPT** (no raw reads needed): NB07d MOFA+ HMP2-only pilot (sample-key joins on cMD-fetch + mart metabolomics + serology); NB09a HMP2 metabolomics CD-vs-HC; NB09b 35-compound cross-cohort metabolomics bridge; NB09c HMP2 metabolite-level corroboration of NB07c cross-feeding; NB09d H3d ecotype-from-metabolites refit; NB10a Kumbhari-only strain-adaptation gene predictor (H3b); NB11 HMP2 serology integration; NB12+ Pillar 4 phage; NB15+ Pillar 5 UC Davis cocktail draft.
  - **Generalizable lesson**: the project's data scope is now provably-sufficient for the science question. The five-line iron-acquisition convergence (NB05 §5g + NB07a §c + NB07 v1.8 + NB07c + NB08a) is at the level of "robustly cross-corroborated" without raw reads, and the deferred raw-read items would refine effect sizes, not change verdicts. v1.9 makes this explicit so that no future revision re-introduces a raw-read-dependent test as a "natural next step."
