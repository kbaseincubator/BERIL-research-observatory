"""NB26k — sampled gene-neighborhood for PUL + mycolic, matching PSII scope.

Cyanobacteriia has 309 species → 27K focal features → tractable.
Bacteroidota has 2,581 species → 723K focal features → blows toPandas.
Mycobacteriaceae has 459 species → ~30K-50K focal features → likely tractable.

Sample down Bacteroidota to 309 random species (matches Cyanobacteriia scope).
Run Mycobacteriaceae full (459) — should fit.
"""
import os, json, time
...
tprint("=== NB26k — sampled PUL + full mycolic gene-neighborhood ===")
...
PUL_CANONICAL = ["K21572", "K21573"]
MYCOLIC_SPECIFIC = ["K11212", "K11211", "K11778", "K11533", "K11534",
                     "K00208", "K20274", "K11782", "K01205", "K00667", "K00507"]

# Sample 309 Bacteroidota species (matching Cyanobacteriia scope)
bact = species[species["phylum"] == "p__Bacteroidota"]
bact_sample = bact.sample(309, random_state=42)["gtdb_species_clade_id"].tolist()
tprint(f"Bacteroidota sample: {len(bact_sample)} of {len(bact)} species")

myco = species[species["family"] == "f__Mycobacteriaceae"]
myco_all = myco["gtdb_species_clade_id"].tolist()
tprint(f"Mycobacteriaceae: {len(myco_all)} species")

HYPOTHESES = [
    ("pul_sampled", "Bacteroidota × PUL canonical (309 sampled)", PUL_CANONICAL, bact_sample),
    ("mycolic_full", "Mycobacteriaceae × mycolic-specific (full)", MYCOLIC_SPECIFIC, myco_all),
]
...
for hyp_id, hyp_label, focal_kos, focal_species in HYPOTHESES:
    ...
    tprint(f"    distinct CDS feat_ids on focal contigs: {len(cf_fids):,}")
    ...
    tprint(f"    Stage 5: {len(contig_features_pd):,}")

    # Stage 6 vectorized
    tprint(f"  [{hyp_id}] Stage 6 vectorized")
    ...
    tprint(f"  [{hyp_id}] per-feature: {len(per_feature):,}")
    tprint(f"  [{hyp_id}] mean fraction MGE neighbors: {per_feature['mge_neighbor_fraction'].mean():.4f}")
    tprint(f"  [{hyp_id}] %% with ≥1 MGE neighbor: {per_feature['any_mge_neighbor'].mean()*100:.2f}%%")
