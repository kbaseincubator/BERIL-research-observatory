...
- **v3** (2026-05-01): NB01 calibration revealed source-disagreement patterns that update the source-of-truth decisions. **lanM**: zero concordance — bakta `product='Lanmodulin'` is the sole source (62 genomes); eggNOG `Preferred_name='lanM'` produces 505 unrelated false positives in gut Bacillota (Streptococcus, Blautia, Enterococcus). **xoxJ**: KO `K02030` is non-specific (46,369 eggNOG-only hits across phyla) — bakta `product` is sole source. **xoxF**: low concordance (418 both / 1,402 bakta-only / 3,272 eggNOG-only). The eggNOG K00114 set is broader and includes many bakta `Putative dehydrogenase XoxF` entries; bakta-only includes lanthanide-dependent ethanol-DH variants. NB02 will use eggNOG K00114 as the primary `xoxF` definition with bakta as an optional union; both subsets reported. **pqq[A-E]**: bakta finds 50-90K bakta-only hits per gene — likely loose regex matching general "PQQ" annotations; NB02 must tighten the bakta regex to `\b(pqqA|pqqB|...)\b` token boundaries before treating bakta as a source. **mxaF (K14028)**: 195 total genomes — eggNOG KO is the reliable source. `lanM_eggnog` column will be excluded from downstream lanmodulin analyses (kept in the matrix only for documenting the false-positive rate).
