"""
Set up Opus + Codex relabel of all 755 negatives.jsonl with the FULL
augmented dossier:
  - basic 8-layer dossier (notebooks/dossier.py)
  - per-gene per-paper PaperBLAST summaries (manuscript-summaries-merged.tsv)
  - extended InterPro / pathway annotations (target_gene_interpro_union.tsv)
  - cross-organism ortholog fitness (target_gene_ortholog_fitness.tsv)

Why this run
------------
The v2 sample reclass (n=40) showed both Opus-tier capability matching
AND augmented evidence shift labels in the negatives:
  negatives both-LLM agree flip rate: 32% (mostly recalcitrant -> improvable_new)
  positives:                          5%  (stable, same as v1)

The original negatives were generated by Sonnet (mid-tier) + Codex 5.5
(frontier) on the basic dossier. That mismatch likely conflated genuine
recalcitrance with Sonnet conservatism. This relabel uses Opus + Codex
5.5 (capability-matched frontier pair) on the augmented dossier, so the
two-LLM agreement filter is much stronger weak supervision.

Output
------
  data/relabel_negatives/
    batches_claude/batch_RN{001..031}/  - 25 dossiers per batch
    batches_codex/batch_RN{001..031}/   - same input, separate prompt suffix
    relabel_keys.tsv                     - orgId, locusId, original verdicts
"""
...
# Reuse renderers from the v2 sample reclass
sys.path.insert(0, str(NB_DIR))
...
OUT_DIR = PROJECT_DATA / "relabel_negatives"
CLAUDE_DIR = OUT_DIR / "batches_claude"
CODEX_DIR = OUT_DIR / "batches_codex"
KEYS_TSV = OUT_DIR / "relabel_keys.tsv"
...
def main() -> None:
    ...
    print(f"negatives: {len(sample)}", file=sys.stderr)
    ...
    print(f"  interpro union rows: {len(interpro):,}", file=sys.stderr)
    ...
    print(f"  ortholog fitness rows: {len(ortho):,}", file=sys.stderr)
    ...
    for bi in range(n_batches):
        ...
        body_lines = [f"# Negatives relabel batch {bid} — {len(chunk)} dossiers "
