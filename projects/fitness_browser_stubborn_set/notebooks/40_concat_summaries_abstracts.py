"""
Concat per-paper Codex abstract-based summaries into
data/manuscript-summaries-abstracts.tsv (4-col schema). Used as a fallback
literature source for papers without PMC full text.
"""
...
SUM_DIR = PROJECT_DATA / "codex_summaries_abstracts"
PER_PAPER_DIR = SUM_DIR / "per_paper"
OUT_PATH = PROJECT_DATA / "manuscript-summaries-abstracts.tsv"
    ...
    with open(OUT_PATH, "w") as out:
        ...
        for tsv in sorted(PER_PAPER_DIR.glob("*.tsv")):
            ...
            for line in content.splitlines():
                ...
                n_rows += 1
    print(f"Files read:  {n_files}", file=sys.stderr)
    print(f"Rows written: {n_rows}  ({n_null} null)", file=sys.stderr)
