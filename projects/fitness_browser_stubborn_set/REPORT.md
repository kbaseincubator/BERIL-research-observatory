...
- **Contamination control** (focal-gene level): BERDL `gene.desc` was used as the original description for the 1,662 rows where it differs from the curator's `new_annotation` (validated to match the FEBA `original_description` 99.8% of the time on 439 overlap rows). For 100 rows where BERDL had been updated to the curator's name, `"uncharacterized protein"` placeholder was substituted; sequence-derived evidence layers (KEGG, SwissProt, SEED, Pfam, PaperBLAST) are unaffected (`notebooks/22_prepare_reann_batches.py`).
- **Partial-leak caveat — partner and neighborhood descriptions**: The dossier shows `gene.desc` for the focal gene's top-10 cofit partners and its ±5 genomic neighbors so the agent can do operon / guilt-by-association reasoning (curators rely on the same signal — Price 2018 reannotation comments routinely cite operon context). The contamination control swap was applied **only to the focal gene's `gene.desc`**, not to its partners or neighbors. If a partner or neighbor happens to also be in the reannotation set, its description in the dossier reflects the curator's *post*-curation name. Population effect: only 1.3% of FB genes were reannotated (1,762 / 137,798), so the per-gene probability that any of its ~15 visible partners/neighbors was curated-and-leaked is small, and the per-row probability that *any* of them leaks an evidentially-useful answer for the focal gene is smaller still. The calibration headline numbers are robust to this; the partial leak is intentional for training (an agent in production will see the world as it is, including prior curator work on neighbors). For a stricter calibration we could extend the swap to every reannotated gene that appears anywhere in the dossier — straightforward to add but unnecessary given the small footprint.
