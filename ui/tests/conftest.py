...
def repository_data(project, completed_project, collection):
    ...
    opportunity_page = WikiPage(
        ...
        source_docs=["docs/discoveries.md"],
        related_collections=["kbase_ke_pangenome"],
        related_pages=["topic.test", "conflict.test", "data.test-product"],
        confidence="medium",
        generated_by="pytest",
        ...
        metadata={
            ...
            "linked_conflicts": ["conflict.test"],
            "linked_products": ["data.test-product"],
            "target_outputs": ["Test ranked opportunity output."],
            "review_routes": ["test_project"],
            "evidence": [{"source": "test_project", "support": "Test support."}],
        },
    )
