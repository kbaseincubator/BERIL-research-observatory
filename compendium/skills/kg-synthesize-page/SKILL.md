...
- Optional: cached page markdown for unchanged page/member hashes.
...
3. Write the deterministic page context pack:
   ```bash
   cd compendium
   uv run compendium page-context kg/<project_id>.kg.yaml --page-id <page_id> --source-root ../projects --out page-contexts
   ```
4. If the page/member hash matches a cached page artifact, reuse the cached markdown. Otherwise ask the
...
6. Write page markdown with:
   ...
   ```bash
   cd compendium
   uv run compendium page-artifact kg/<project_id>.kg.yaml --page-id <page_id> --markdown <draft.md> --out pages --model <model-id> --prompt-hash <prompt-hash>
   ```
