...
def test_adp1_tracer_fixture_validates_as_project_kg() -> None:
    ...
    assert {card.evidence[0].source_project for card in cards} == {
        "adp1_deletion_phenotypes",
        "adp1_triple_essentiality",
    }
    assert all("entity:adp1" in card.entities for card in cards)
    assert all("topic:adp1-carbon-fitness" in card.topics for card in cards)
...
def test_adp1_tracer_evidence_exact_text_resolves() -> None:
    ...
    for card in result.records:
        ...
        source = PROJECTS / evidence.source_project / evidence.source_doc
        source_text = source.read_text(encoding="utf-8")

        assert evidence.quote in source_text, card.id
