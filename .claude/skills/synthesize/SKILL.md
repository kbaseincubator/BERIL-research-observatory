...
- **Claims ledger (`## Claims`)**: optional. Formalize each Key Finding as a falsifiable claim with typed evidence pointers (`notebook` / `query` / `figure` / `paper` / `web` / `docs`) and author-written confidence/status. Notebook locators are project-relative; `#cell-N` is one-based. Use `[stream: id]` only when artifacts genuinely represent a named dataset/method group. Without explicit stream metadata, multiple artifacts remain one default stream. A `query: q:{id}` pointer resolves only if that `q:{id}` was registered in `projects/{project_id}/journal.jsonl` while the query ran (`/berdl-query` § Registering a Query as Evidence); otherwise it resolves as `query-not-recorded` and grounds nothing. The CLI computes **resolved artifact support** and **confidence mismatch**; it does not prove status or scientific independence. `/berdl-review` and `/submit` read this ledger.
...
If a `query:` pointer comes back `unresolved` with reason `query-not-recorded`, that query was never registered while it ran. **Do not backfill it** by running `beril capture-event` now from the SQL in a notebook — a record written at synthesis time is reconstructed, which is exactly the failure the journal exists to prevent. Either cite the notebook cell that ran the query instead, or leave the pointer unresolved and let the confidence mismatch stand.
