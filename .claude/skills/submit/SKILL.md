...
- **Calibrated trust (claims ledger)**: if `REPORT.md` has a `## Claims` section, run `beril claims summary {project_id}` (read-only — it recomputes from REPORT.md and does **not** write `claims.json`). It prints the claim tally and a tier-mismatch count. If any claim is flagged, print `WARN  {N} claim(s) assert high/medium confidence on a single/no re-runnable source — see claims.json (computed, advisory; reviewers decide if the gap is fatal)`. If there is no `## Claims` section, skip silently. This is an advisory signal only — it never blocks submission, and it surfaces a word/count, never a numeric confidence.
