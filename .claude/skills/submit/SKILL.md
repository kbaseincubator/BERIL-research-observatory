...
- **Plan deviations**: if `projects/{project_id}/plan_deviations.jsonl` exists and is non-empty, print `WARN  {N} analysis file(s) written without a matching plan_approval — see plan_deviations.jsonl` followed by each record's `path` and `at`. Advisory only, never a blocker: the hook is a witness, and submitting unplanned work is fine as long as the write-up says so. Prompt the author to confirm the departures are disclosed in `RESEARCH_PLAN.md` Revision History or `REPORT.md`. If the file is absent or empty, skip silently.
- **Plan approval provenance**: if `beril.yaml` has `plan_approval.via`, print `INFO  plan approval recorded via {via}` — `terminal` (a human ran `beril approve` at a terminal) or `agent-relayed` (the agent recorded an approval the user gave in conversation; the CLI did not witness it). Provenance, not a fault; advisory only, never a blocker. Absent: skip silently.
