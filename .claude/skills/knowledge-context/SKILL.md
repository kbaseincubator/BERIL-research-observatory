...
For a local OpenViking server, copy `knowledge/openviking/ov.conf.example` to `~/.openviking/ov.conf`, replace the placeholder local server key, and set the OpenRouter key in the embedding and VLM sections.
